...
from sklearn.model_selection import (
    StratifiedKFold, GridSearchCV, StratifiedGroupKFold
)
...
if __name__ == "__main__":
    try:
        ...
        test_data_tag = get_data_name(test_csv)  # Used for test output naming
        ...
        try:
            ...
            test_results = []
            test_out_path = os.path.join(RESULTS_ROOT, "Test Results", data)
            print("\n" + "#" * 50)
            print("Running Multi-Subject Diagnosis")
            print("#" * 50)


            model_root_path = RESULTS_ROOT

            for index, row in test_df.iterrows():
                # Probabilistic Diagnosis
                final_pred_str, binary_comp_results = final_diagnosis_probabilistic(
                    row.drop(["Image_ID", "Subject", "Diagnosis"], errors="ignore"), pca_num, model_root_path)

                result_entry = {'Subject': row.get('Subject', f'idx_{index}'), 'Image_ID': row.get('Image_ID', 'N/A'),
                ...
                print(f"Subject {row.get('Subject', f'idx_{index}')}: Predicted -> {final_pred_str}")

            test_results_df = pd.DataFrame(test_results)
            os.makedirs(test_out_path, exist_ok=True)

            # Output file name
            test_results_file = f"{test_out_path}/Test_Prediction_{test_data_tag}.csv"
            test_results_df.to_csv(test_results_file, index=False)
            print(f"\n{test_results_file}")  # Display output file

            # Check accuracy and print report
            check_accuracy(test_out_path, test_data_tag, data)
