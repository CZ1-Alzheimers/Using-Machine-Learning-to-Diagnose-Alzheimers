...
def run_pca_and_models_cv(stage_name, X_sub_clean_df, y_sub, groups, pca_num, label_a, label_b, initial_scaler=None):
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X_final_df, y_sub, groups)):
        ...
        last_history = history # Save for plotting
        last_test_loss, _ = model.evaluate(X_test_fold, y_test_fold, verbose=0)

        y_proba = model.predict(X_test_fold, verbose=0)
        y_pred = (y_proba > 0.5).astype(int).flatten()
        
        # Store data for ROC plot
        nn_roc_data.append((y_test_fold.to_numpy(), y_proba.flatten()))
        
        # Get all metrics
        acc, auc, sens, spec, f1, tn, fp, fn, tp = evaluate_model(
            y_test_fold.to_numpy(), y_pred, y_proba
        )
        nn_fold_metrics.append({
            'acc': acc, 'auc': auc, 'sens': sens, 'spec': spec, 'f1': f1,
            'tn': tn, 'fp': fp, 'fn': fn, 'tp': tp
        })

    # Calculate mean, std dev, and 95% CI for NN
    ...
    # Print NN Results
    print(f"\nNeural Network ({stage_name}) 5-Fold CV Results (Mean (Std) [95% CI]):")
    print(f"  Accuracy:    {mean_metrics['acc']:.3f} ({std_metrics['acc']:.3f}) [{ci_metrics['acc_ci_low']:.3f} - {ci_metrics['acc_ci_high']:.3f}]")
    print(f"  AUC:         {mean_metrics['auc']:.3f} ({std_metrics['auc']:.3f}) [{ci_metrics['auc_ci_low']:.3f} - {ci_metrics['auc_ci_high']:.3f}]")
    print(f"  Sensitivity: {mean_metrics['sens']:.3f} ({std_metrics['sens']:.3f}) [{ci_metrics['sens_ci_low']:.3f} - {ci_metrics['sens_ci_high']:.3f}]")
    print(f"  Specificity: {mean_metrics['spec']:.3f} ({std_metrics['spec']:.3f}) [{ci_metrics['spec_ci_low']:.3f} - {ci_metrics['spec_ci_high']:.3f}]")
    ...
    # Plot Loss (from the last fold)
    if last_history:
        plt.figure(figsize=(8, 5))
        plt.plot(last_history.history['loss'], label='Training Loss')
        plt.plot(last_history.history['val_loss'], label='Validation Loss')
        plt.axhline(y=last_test_loss, color='r', linestyle='--', label=f'Last Fold Test Loss = {last_test_loss:.4f}')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title(f'NN Loss - {label_a} vs {label_b} - PCA: {stage_name}')
        plt.legend()
        plt.grid(True)
        os.makedirs(results_path, exist_ok=True)
        plt.savefig(f"{results_path}/{label_a}_vs_{label_b}_{stage_name}_NN_Loss_({pca_num}_dim).png", dpi=300,
                    bbox_inches='tight')
        plt.close()
    ...
    all_model_roc_data['NeuralNet'] = nn_roc_data
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Load dataset
        pca_num = 50
        filename = "ADNI-Oasis_dataset.csv"
